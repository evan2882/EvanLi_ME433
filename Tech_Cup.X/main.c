...
void __ISR(_TIMER_4_VECTOR, IPL5SOFT) Timer4ISR(void) { //100Hz
  _CP0_SET_COUNT(0);
  if(flag==1){
    if (com < c/2/2){
        e = c/2/2 - com;
        speed = maxspeed - (2399/(c/2/2))*e; // when the com is all the way over, the motor is all off
        if(speed > maxspeed){
            speed = maxspeed;
        }
        if(speed < 0){
...
void startup() {
    ...
    T4CONbits.TCKPS = 0b111;     // Timer3 prescaler  (1:256)
  	PR4 = 1874;                    //             set period register   10hz
